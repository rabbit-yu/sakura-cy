...
func (p EngineParams) SetProxyAuthorization(ProxyAuthorization string) {
	cProxyAuthorization := C.CString(ProxyAuthorization)
	C.Cronet_EngineParams_user_agent_set(p.ptr, cProxyAuthorization)
	C.free(unsafe.Pointer(cProxyAuthorization))
}
...
func (p EngineParams) ProxyServer() string {
	return C.GoString(C.Cronet_EngineParams_proxy_rules_get(p.ptr))
}
